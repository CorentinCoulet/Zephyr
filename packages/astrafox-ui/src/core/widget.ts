...
import type { AstrafoxConfig, AstrafoxLogo, AstrafoxPersona, AstrafoxOpenAnimation, AstrafoxTriggerAnimation } from "./config";
import { resolveConfig, ASTRAFOX_DEFAULTS } from "./config";
...
import { AstrafoxWSClient, type AstrafoxWSMessage } from "./ws-client";

// ─── Types ───────────────────────────────────────────────────

export interface AstrafoxMessage {
...
export interface AstrafoxWidgetEvents {
  onReady?: () => void;
...
export class AstrafoxWidget {
  private config: ReturnType<typeof resolveConfig>;
  private events: AstrafoxWidgetEvents;
  private el: HTMLElement | null = null;
  private wsClient: AstrafoxWSClient | null = null;
  private isOpen = false;
  private messages: AstrafoxMessage[] = [];
  private logo: ResolvedLogo;
  private persona: ResolvedPersona;
  private mounted = false;

  constructor(userConfig: AstrafoxConfig, events: AstrafoxWidgetEvents = {}) {
  ...
  mount(container?: HTMLElement | string | null): void {
    ...
    this.el.className = `astrafox-root ${this.positionClass()}`;
    this.el.setAttribute("data-astrafox", "");
    this.applyThemeVars();
    this.applyPanelVars();
  ...
  setTheme(theme: AstrafoxConfig["theme"]): void {
  ...
  setAccentColor(color: string): void {
    ...
    this.el?.style.setProperty("--astrafox-accent", color);
    this.updateAvatars();
    this.updateChatAvatar();
  }

  /** Change open animation */
  setOpenAnimation(anim: AstrafoxOpenAnimation): void {
  ...
  setTriggerAnimation(anim: AstrafoxTriggerAnimation): void {
    this.config = { ...this.config, triggerAnimation: anim };
    const trigger = this.el?.querySelector(".astrafox-trigger") as HTMLElement;
    if (trigger) {
  ...
  private renderHTML(): string {
    const sizeClass = `astrafox-${this.config.panel.size || "md"}`;
    ...
    const backdrop = this.config.panel.backdrop
      ? `<div class="astrafox-backdrop"></div>`
      : "";

    return `
      ${backdrop}
      <button class="astrafox-trigger ${sizeClass}" style="${triggerAnimCSS}" aria-label="Astrafox Assistant">
        <div class="astrafox-trigger-avatar">${triggerContent}</div>
        ${this.config.showBadge ? '<div class="astrafox-badge"></div>' : ""}
      </button>
      <div class="astrafox-panel">
        <div class="astrafox-header">
          <div class="astrafox-header-avatar">${headerContent}</div>
          <div class="astrafox-header-info">
            <div class="astrafox-header-title">Astrafox</div>
            <div class="astrafox-header-sub">${
              this.config.language === "fr" ? "Assistant Navigation" : "Navigation Assistant"
            }</div>
          </div>
          <button class="astrafox-close" aria-label="${this.config.language === "fr" ? "Fermer" : "Close"}">✕</button>
        </div>
        <div class="astrafox-messages"></div>
        <div class="astrafox-suggestions"></div>
        <div class="astrafox-input-wrap">
          <textarea class="astrafox-input" rows="1" placeholder="${this.escapeAttr(ph)}"></textarea>
          <button class="astrafox-send" aria-label="${this.config.language === "fr" ? "Envoyer" : "Send"}">
  ...
  private bindEvents(): void {
    if (!this.el) return;
    const trigger = this.el.querySelector(".astrafox-trigger")!;
    const close = this.el.querySelector(".astrafox-close")!;
    const input = this.el.querySelector(".astrafox-input") as HTMLTextAreaElement;
    const send = this.el.querySelector(".astrafox-send")!;
    const backdrop = this.el.querySelector(".astrafox-backdrop");
  ...
  private setOpenState(open: boolean): void {
    ...
    const panel = this.el?.querySelector(".astrafox-panel") as HTMLElement;
    const trigger = this.el?.querySelector(".astrafox-trigger") as HTMLElement;
    const backdrop = this.el?.querySelector(".astrafox-backdrop") as HTMLElement;

    if (!panel) return;

    if (open) {
      panel.classList.add("astrafox-visible");
      const animName = getOpenAnimationName(this.config.openAnimation);
      if (animName !== "none") {
        panel.style.animation = `${animName} 0.3s ease-out forwards`;
      }
      trigger?.classList.add("astrafox-open");
      backdrop?.classList.add("astrafox-visible");
      setTimeout(() => {
        (this.el?.querySelector(".astrafox-input") as HTMLTextAreaElement)?.focus();
      }, 200);
    } else {
      ...
      if (animName !== "none") {
        ...
        panel.addEventListener("animationend", () => {
          panel.classList.remove("astrafox-visible");
          panel.style.animation = "";
        }, { once: true });
      } else {
        panel.classList.remove("astrafox-visible");
      }
      trigger?.classList.remove("astrafox-open");
      backdrop?.classList.remove("astrafox-visible");
    }
  ...
  private addMessage(role: AstrafoxMessage["role"], text: string, expression?: string): void {
    const msg: AstrafoxMessage = { role, text, expression, timestamp: Date.now() };
    this.messages.push(msg);

    const container = this.el?.querySelector(".astrafox-messages");
    if (!container) return;

    const div = document.createElement("div");
    div.className = `astrafox-msg astrafox-${role}`;
  ...
  private showTyping(): void {
    const container = this.el?.querySelector(".astrafox-messages");
    if (!container || container.querySelector(".astrafox-typing")) return;
    const typing = document.createElement("div");
    typing.className = "astrafox-typing";
  ...
  private hideTyping(): void {
    this.el?.querySelector(".astrafox-typing")?.remove();
  }

  private showSuggestions(items: string[]): void {
    const container = this.el?.querySelector(".astrafox-suggestions");
    if (!container) return;
    container.innerHTML = items
      .map((s) => `<button class="astrafox-suggestion">${this.escapeHTML(s)}</button>`)
      .join("");
    container.querySelectorAll(".astrafox-suggestion").forEach((btn) => {
  ...
  private applyPanelVars(): void {
    ...
    this.el.style.setProperty("--astrafox-panel-width", `${p.width}px`);
    this.el.style.setProperty("--astrafox-panel-height", `${p.height}px`);
    this.el.style.setProperty("--astrafox-panel-radius", `${p.borderRadius}px`);
    this.el.style.setProperty("z-index", String(p.zIndex));
  }

  private positionClass(): string {
    const map: Record<string, string> = {
      "bottom-right": "astrafox-pos-br",
      "bottom-left": "astrafox-pos-bl",
      "top-right": "astrafox-pos-tr",
      "top-left": "astrafox-pos-tl",
    };
    return map[this.config.panel.position] || "astrafox-pos-br";
  }
  ...
  private updateAvatars(): void {
    ...
    const triggerAv = this.el.querySelector(".astrafox-trigger-avatar");
    const headerAv = this.el.querySelector(".astrafox-header-avatar");
  ...
  private connectWS(): void {
    this.wsClient = new AstrafoxWSClient({
  ...
  private handleWSMessage(msg: AstrafoxWSMessage): void {
    switch (msg.type) {
      case "welcome":
