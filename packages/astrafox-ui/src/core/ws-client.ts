/**
 * 🔌 Astrafox WebSocket Client
 *
 * Handles connection to the Astrafox backend with auto-reconnect,
 * session management, and typed events.
 */

export interface AstrafoxWSMessage {
...
export interface AstrafoxWSOptions {
  server: string;
  apiKey?: string;
  sessionId?: string;
  onMessage: (msg: AstrafoxWSMessage) => void;
...
export class AstrafoxWSClient {
  private ws: WebSocket | null = null;
  private options: Required<AstrafoxWSOptions>;
  private sessionId: string = "";
  private reconnectAttempts = 0;
  private reconnectTimer: ReturnType<typeof setTimeout> | null = null;
  private destroyed = false;

  constructor(options: AstrafoxWSOptions) {
    this.options = {
      reconnectDelay: 3000,
  ...
  connect(): void {
    ...
    if (!this.options.server) {
      console.warn("[Astrafox] No server URL configured.");
      return;
    }
    ...
    this.ws.onmessage = (event) => {
      try {
        const data: AstrafoxWSMessage = JSON.parse(event.data);
