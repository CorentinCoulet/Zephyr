...
Embeddable AI assistant widget for any web project. Import `<Astrafox />` into React, Vue, or vanilla JS — configure everything via a single config file.
...
Create a `astrafox.config.ts` in your project:
...
export default defineAstrafoxConfig({
  ...
  // Appearance
  logo: 'astrafox-default',    // Built-in or custom URL
...
| Value | Description |
|---|---|
| `'astrafox-default'` | Official Astrafox fox logo |
...
```tsx
import { useAstrafox } from '@astrafox/ui/react';

function MyButton() {
  const { send, toggle, setTheme, setPersona, isOpen } = useAstrafox();

  return (
    <button onClick={() => toggle()}>
      {isOpen() ? 'Close' : 'Open'} Astrafox
    </button>
  );
}
```
...
```js
import { init, getInstance, destroy } from '@astrafox/ui';
...
Create `astrafox.server.yaml` in your project to configure the LLM backend:
...
| Import | Contents |
|---|---|
| `@astrafox/ui` | Everything: widget, types, config, themes |
| `@astrafox/ui/react` | `<Astrafox />`, `AstrafoxProvider`, `useAstrafox()` |
| `@astrafox/ui/vue` | `<Astrafox />` Vue component |
| `@astrafox/ui/config` | `defineAstrafoxConfig()`, types |
| `@astrafox/ui/themes` | Theme objects and helpers |
