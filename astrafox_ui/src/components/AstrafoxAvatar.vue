<template>
  <div class="astrafox-avatar" :class="`size-${size}`">
    <!-- Glow effect -->
    <div v-if="glow" class="astrafox-glow"></div>
    ...
    <div
      ...
      :style="{ backgroundColor: 'var(--astrafox-surface)' }"
    >
      <!-- Fox face SVG -->
      <svg
        :width="svgSize"
      ...
      >
        ...
        <g v-if="expression === 'thinking'" class="animate-pulse">
          <circle cx="24" cy="46" r="1.5" fill="var(--astrafox-accent)" />
          <circle cx="32" cy="46" r="1.5" fill="var(--astrafox-accent)" />
          <circle cx="40" cy="46" r="1.5" fill="var(--astrafox-accent)" />
        </g>
      </svg>
    </div>
  </div>
...
const earColor = computed(() => "var(--astrafox-primary)");
...
const borderColor = computed(() => {
  const map = {
    ...
    neutral: "border-astrafox-accent",
    speaking: "border-astrafox-primary",
  };
  return map[props.expression] || "border-astrafox-accent";
});
